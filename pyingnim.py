...
def put_tags(image, tags) -> None:
    ...
    for tag in get_real_tags(font, tags, 700):
        ...
        tImg = Image.new("RGBA", (ln + (TAG_IN_MARGIN * 2), TAG_FONT_SIZE + TAG_IN_MARGIN), (40, 40, 40, 240))
        canvas = ImageDraw.Draw(tImg)
        canvas.text((TAG_IN_MARGIN, TAG_IN_MARGIN / 3), tag, fill = (235, 235, 235), font = font)
...
def put_mark(image, canvas, text) -> None:
    ...
    canvas.text((LEFT_MARGIN + 10 + star.size[0], 450), text, fill = (255, 255, 255), font = font)
